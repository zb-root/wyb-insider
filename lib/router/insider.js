    ...
    routes.after_update = function (opts, cb, next) {
      ...
      if (!opts.err && state != undefined) {
        service.insider.findOne({_id: id}, function (err, doc) {
          if (doc) {
            if (state == 1) {
              service.insider.addPermission({mobile: doc.mobile, name: doc.name, idcard: doc.idcard, creator: opts.headers.acl_user}, function (err, doc) {
                if (err || doc && doc.err) return
                service.insider.update({_id: id}, {user: doc._id}, function (err, doc) {
                })
              })
            } else {
              service.insider.removePermission(doc.user.toString(),function(err, doc){
