...
module.exports = function (service, opts = {}) {
  ...
  service.onReady().then(() => {
    ...
    routes.before_create = function (opts, cb, next) {
      ...
      opts.data = _.pick(data, ['name', 'mobile', 'idcard', 'state', 'department'])
    ...
    routes.after_create = function (opts, cb, next) {
      ...
      if (!opts.err && data.state == 1) {
        service.insider.addPermission({
          id: data.mobile.toString(),
          creator: opts.headers.acl_user
        }, function (err, doc) {
          opts.doc.user = doc._id
    ...
    routes.after_update = function (opts, cb, next) {
      ...
      if (!opts.err && state != undefined) {
        service.insider.findOne({_id: id}, function (err, doc) {
          if (doc) {
            if (state == 1) {
              service.insider.addPermission({id: doc.mobile, creator: opts.headers.acl_user}, function (err, doc) {
                if (err || doc && doc.err) return
                service.insider.update({_id: id}, {user: doc._id}, function (err, doc) {
                })
              })
            } else {
              service.insider.removePermission(doc.user.toString())
            }
          }
        })
      }
